    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          ...
          <span className="ml-auto bg-amber-50 text-amber-600 border border-amber-100 text-xs px-3 py-1 rounded-full font-medium">
            {terms.length}개
          </span>
        </div>
      </header>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 h-11 rounded-xl border border-slate-200 bg-white text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
          />
        </div>
      </div>
        ...
        {!loading && filtered.length > 0 && (
          <div className="grid gap-2">
            {filtered.map((t, i) => (
              <div
                key={i}
                className="bg-white border border-slate-200 rounded-xl px-5 py-3.5 flex items-center justify-between hover:shadow-sm transition-all"
              >
                <div className="flex items-center gap-3">
